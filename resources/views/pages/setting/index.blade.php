          ...
          <div class="row">
            <div class="col-md-6 border-right-2 border-right-blue-400">
              <div class="form-group row">
                ...
                <div class="col-lg-9">
                  <input name="school_name" value="{{ $setting['school_name'] }}" required type="text"
                    class="form-control" placeholder="Name of School">
                </div>
              </div>
              <div class="form-group row">
                <label for="current_session" class="col-lg-2 col-form-label font-weight-semibold">العام الحالي<span
                    class="text-danger">*</span>
                </label>
                <div class="col-lg-9">
                  {{-- <select data-placeholder="Choose..." required name="current_session" id="current_session"
                    ...
                    <option value=""></option>
                    @for($y=date('Y', strtotime('- 3 years')); $y <= date('Y', strtotime('+ 1 years')); $y++) <option {{
                  ...
                  <select class="form-control p-1 mr-sm-2" name="current_session" id="current_session" required>
                    <option selected disabled>{{trans('parents.Choose')}}...</option>
                    @php
                    $current_year = date("Y");
                    @endphp
                    @for($year=$current_year -3; $year != $current_year +2 ;$year++)
                    <option value="{{ ($year-1).'-'.($year) }}" {{ ($setting['current_session']==($year-1).'-'.($year))
                      ? 'selected' : '' }}>
                      {{ ($year-1).'-'.($year) }}
                    </option>
                    @endfor
              ...
              <div class="form-group row">
                ...
                <div class="col-lg-9">
                  <input name="school_title" value="{{ $setting['school_title'] }}" type="text" class="form-control"
              ...
              <div class="form-group row">
                ...
                <div class="col-lg-9">
                  <input name="end_second_term" value="{{ $setting['end_second_term'] }}" type="text"
                    class="form-control date-pick" placeholder="Date Term Ends">
                </div>
              </div>
              <div class="form-group row">
                ...
                <div class="col-lg-9">
                  <div class="mb-3">
                    <img style="width: 100px" height="100px"
                      src="{{ URL::asset('attachments/logo/'.$setting['logo']) }}" alt="{{ $setting['logo'] }}">
                  </div>
                  <input type="file" name="logo" accept="image/*" class="file-input" data-show-caption="false"
