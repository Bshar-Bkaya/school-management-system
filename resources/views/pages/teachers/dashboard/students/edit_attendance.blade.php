      ...
      <div class="modal-body">
        <form action="{{route('attendance.edit','test')}}" method="post">
          ...
          <label class="block text-gray-500 font-semibold sm:border-r sm:pr-4">
            <input name="attendences" {{ $student->attendance()->first()->attendence_status == 1 ? 'checked' : '' }}
            class="leading-tight" type="radio" value="presence">
            <span class="text-success">حضور</span>
          </label>

          <label class="ml-4 block text-gray-500 font-semibold">
            <input name="attendences" {{ $student->attendance()->first()->attendence_status == 0 ? 'checked' : '' }}
